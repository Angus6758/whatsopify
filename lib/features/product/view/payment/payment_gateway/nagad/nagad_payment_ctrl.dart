...
class NagadPaymentCtrlNotifier extends StateNotifier<String> {
  ...
  Future initializePayment(BuildContext context) async {
    try {
      ...
      final encryptedData =_encrypter.encryptWithPublicKey(sensitiveData, _cred.publicKey);
      final signature =_encrypter.signWithPrivateKey(sensitiveData, _cred.privateKey);
  ...
  Future<String?> _confirmPayment(NagadInitDecryptedData data) async {
    ...
    /// encrypt sensitive data with public key
    /// and sign with private key
    final encryptedData =_encrypter.encryptWithPublicKey(sensitive, _cred.publicKey);
    final signature = _encrypter.signWithPrivateKey(sensitive, _cred.privateKey);
